    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-6 flex items-center justify-between">
                <div>
                    <h3 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Edit Property') }}</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('Update the details of this property.') }}</p>
                </div>
            ...
            <form action="{{ route('admin.properties.update', $property->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-md rounded-xl">
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <x-text-input id="title" class="mt-1 block w-full" type="text" name="title" :value="old('title', $property->title)" required autofocus />
                            <x-input-error :messages="$errors->get('title')" class="mt-2" />
                        </div>

                        <div class="md:col-span-2">
                            <x-input-label for="description" :value="__('Description')" />
                            <textarea id="description" name="description" rows="5" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:focus:ring-indigo-600">{{ old('description', $property->description) }}</textarea>
                        ...
                        <div>
                            ...
                            <x-text-input id="price" class="mt-1 block w-full" type="number" step="0.01" min="0" name="price" :value="old('price', $property->price)" required />
                            <x-input-error :messages="$errors->get('price')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="city" :value="__('City')" />
                            <x-text-input id="city" class="mt-1 block w-full" type="text" name="city" :value="old('city', $property->city)" />
                            <x-input-error :messages="$errors->get('city')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="state" :value="__('State / Province')" />
                            <x-text-input id="state" class="mt-1 block w-full" type="text" name="state" :value="old('state', $property->state)" />
                            <x-input-error :messages="$errors->get('state')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="area" :value="__('Area (m²)')" />
                            <x-text-input id="area" class="mt-1 block w-full" type="number" step="0.01" min="0" name="area" :value="old('area', $property->area)" />
                            <x-input-error :messages="$errors->get('area')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="bedrooms" :value="__('Bedrooms')" />
                            <x-text-input id="bedrooms" class="mt-1 block w-full" type="number" min="0" name="bedrooms" :value="old('bedrooms', $property->bedrooms)" />
                            <x-input-error :messages="$errors->get('bedrooms')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="bathrooms" :value="__('Bathrooms')" />
                            <x-text-input id="bathrooms" class="mt-1 block w-full" type="number" min="0" name="bathrooms" :value="old('bathrooms', $property->bathrooms)" />
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-md rounded-xl">
                    ...
                    <div class="p-6 space-y-6">
                        <div>
                            ...
                            <div class="mt-2 flex items-center gap-6">
                                <label class="inline-flex items-center">
                                    <input type="radio" name="status" value="for sale" {{ old('status', $property->status) === 'for sale' ? 'checked' : '' }} class="rounded-full border-gray-300 text-blue-600 shadow-sm focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900">
                                    <span class="ms-2 text-sm text-gray-700 dark:text-gray-300">{{ __('For Sale') }}</span>
                                </label>
                                <label class="inline-flex items-center">
                                    <input type="radio" name="status" value="for rent" {{ old('status', $property->status) === 'for rent' ? 'checked' : '' }} class="rounded-full border-gray-300 text-sky-600 shadow-sm focus:ring-sky-500 dark:border-gray-700 dark:bg-gray-900">
                        ...
                        <div class="flex items-center gap-8">
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $property->is_featured) ? 'checked' : '' }} class="rounded border-gray-300 text-amber-500 shadow-sm focus:ring-amber-500 dark:border-gray-700 dark:bg-gray-900">
                                <span class="ms-2 text-sm text-gray-700 dark:text-gray-300">{{ __('Featured') }}</span>
                            </label>
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $property->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-emerald-500 shadow-sm focus:ring-emerald-500 dark:border-gray-700 dark:bg-gray-900">
                    ...
                    <div class="p-6">
                        @if ($property->main_image)
                            <div class="mb-6">
                                ...
                                <img src="{{ asset('storage/' . $property->main_image) }}" alt="{{ $property->title }}" class="mt-2 w-48 h-36 rounded-lg object-cover">
                            </div>
                        @endif
                            ...
                            <div class="mb-6">
                                ...
                                <div class="mt-2 flex flex-wrap gap-3">
                                    @foreach ($property->images as $image)
                                        <img src="{{ asset('storage/' . $image->image_path) }}" alt="{{ $property->title }}" class="w-24 h-20 rounded-lg object-cover">
                                    @endforeach
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <x-input-label for="main_image" :value="__('Replace Main Image')" />
                            ...
                            <div>
                                <x-input-label for="additional_images" :value="__('Add Additional Images')" />
                ...
                <div class="flex items-center justify-end gap-4">
                    ...
                    <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-5 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-indigo-700">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
